...
class BulkOperationsController
{
    ...
    /**
     * Execute query and return results directly
     * This method bridges the gap between the expected behavior and actual Database class
     */
    ...
    public function processQueue()
    {
        ...
        $operations = $this->executeQuery($query);
    ...
    public function createTemplate($userId, $name, $description, $templateData, $isPublic = false)
    {
        try {
            ...
            $existing = $this->executeQuery($existingQuery);
            ...
            if (!empty($existing)) {
                ...
                $query = "INSERT INTO bulk_templates 
                          (user_id, name, description, template_data, is_public, usage_count) 
                          VALUES (" . (int)$userId . ", '" . addslashes($name) . "', '" . addslashes($description) . "', '" . addslashes(json_encode($templateData)) . "', " . ($isPublic ? 1 : 0) . ", 1)";
            }
